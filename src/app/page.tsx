...
export default function Home() {
  ...
  // pinned topics — lưu localStorage
  ...
  useEffect(() => { void fetchAllVocab(); }, [fetchAllVocab]);

  // aggregate lessons directly from vocabulary table
  ...
  const handleDeleteLesson = useCallback(async (lessonName: string) => {
    try {
      const res = await fetch(`/api/lessons?name=${encodeURIComponent(lessonName)}`, { method: 'DELETE' });
  ...
  const handleDeleteTopic = useCallback(async (topicName: string) => {
    try {
      const res = await fetch(`/api/topics?name=${encodeURIComponent(topicName)}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Xóa thất bại');
  ...
  const handleRandom50 = useCallback(async () => {
    setVocabLoading(true);
    try {
      // lấy tất cả từ có gắn lesson, xáo ngẫu nhiên, chọn 50
      const res = await fetch('/api/vocabulary?hasLesson=1');
      const json = await res.json();
      if (json.data?.length) {
  ...
  return (
    <main className="min-h-screen selection:bg-red-200/40 selection:text-white pb-24 relative overflow-x-hidden">
      ...
      <div className="fixed inset-0 -z-20" aria-hidden>
        <Image src={BG_PHOTOS[next].src} alt="" fill sizes="100vw" className="object-cover object-center" unoptimized />
        <div className={`absolute inset-0 transition-opacity duration-900 ${fading ? 'opacity-0' : 'opacity-100'}`}>
          <Image src={BG_PHOTOS[current].src} alt="" fill priority sizes="100vw" className="object-cover object-center" unoptimized />
        </div>
      </div>
      ...
      <header className="fixed top-0 inset-x-0 z-50 bg-slate-900/55 backdrop-blur-2xl border-b border-white/10">
        <div className="max-w-5xl mx-auto px-5 h-14 flex items-center justify-between gap-3">
              ...
              <button
                onClick={() => {
                  if (viewMode !== 'flashcard') {
                    setViewMode('flashcard'); // thoát mode → về flashcard
                  } else {
                    handleBackToHome();       // đang flashcard → về home
                  }
                }}
          ...
          {/* Mode tabs — inside a lesson OR topic with vocab */}
          {isStudying && (
            <div className="hidden sm:flex bg-white/10 backdrop-blur p-1 rounded-xl gap-0.5">
              {tabs
                .filter(tab => activeLesson || tab.id !== 'grid') // grid chỉ cho lesson
                .map(tab => (
                ...
                return (
                  <div key={topic.name} className="group relative shrink-0">
                    <button onClick={() => handleSelectTopic(topic.name)}
                      ...
                      {isPinned && (
                        <Pin className={`w-2.5 h-2.5 shrink-0 ${isActive ? 'text-white/70' : 'text-amber-400'}`} />
                      )}
                      {topic.name}
      ...
      {isStudying && (
        <div className="sm:hidden fixed top-25 inset-x-0 z-30 px-4 pt-2 pb-2 bg-slate-950/70 backdrop-blur-xl border-b border-white/10">
          <div className="flex bg-white/10 p-1 rounded-xl gap-0.5">
            {tabs
              .filter(tab => activeLesson || tab.id !== 'grid')
              .map(tab => (
                <button key={tab.id} onClick={() => setViewMode(tab.id)}
      ...
      <div className="max-w-4xl mx-auto px-4 pt-28 pb-8">
        ...
        {/* ── Study area (lesson or topic selected) ── */}
        {isStudying ? (
          vocabLoading ? (
          ...
          <div className="space-y-6 animate-fade-up">
            ...
            <section>
              <div className="flex items-center gap-2 mb-4">
                ...
                {!lessonsLoading && (
                  <span className="text-xs text-white/40 font-medium">{lessons.length} bài</span>
                )}
              </div>
              ...
              {lessonsLoading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {lessons.map((lesson, i) => (
                    <div
                      key={lesson.name}
                      className={`group relative bg-linear-to-br ${LESSON_COLORS[i % LESSON_COLORS.length]}
                        border backdrop-blur-md rounded-2xl p-4
                        hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200`}
                    >
                      {/* action buttons — hiện khi hover, ẩn khi đang edit */}
                      {editingLesson !== lesson.name && (
                        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <button
                            onClick={(e) => { e.stopPropagation(); setEditingLesson(lesson.name); setEditLessonValue(lesson.name); }}
                            className="w-6 h-6 rounded-full bg-black/30 hover:bg-blue-500 text-white/50 hover:text-white flex items-center justify-center transition-all"
                            title="Đổi tên"
                          >
                            <Pencil className="w-3 h-3" />
                          </button>
                          <button
                            onClick={(e) => { e.stopPropagation(); void handleDeleteLesson(lesson.name); }}
                            className="w-6 h-6 rounded-full bg-black/30 hover:bg-red-500 text-white/50 hover:text-white flex items-center justify-center transition-all"
                            title="Xóa bài học"
                          >
                      ...
                      {editingLesson === lesson.name ? (
                        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                          <input
                            ...
                            onKeyDown={async (e) => {
                              if (e.key === 'Enter') {
                                await handleRenameLesson(lesson.name, editLessonValue);
